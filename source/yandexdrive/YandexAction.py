...
class YandexAction:
    ...
    def create_folder(self, path):
        ...
        """
        response = requests.get(f"{self.Y_URL}?path={path}",
                                headers=self.headers)
        if response.status_code == 404:
            requests.put(f"{self.Y_URL}?path={path}",
                         headers=self.headers)
    ...
    def delete_backup_on_cloud(self, backup_name):
        ...
        if confirm_response.status_code == 404:
            ...
            sys.exit(0)
        print("Error: backup doesn't deleted", confirm_response.status_code)
    ...
    def upload(self, path_to_file, folder_name):
        ...
        check_response = requests.get(
            f"{self.Y_URL}?path={full_path}", headers=self.headers
        )
        upload_file = True
    ...
    def download(self, backup_name, path_on_machine):
        ...
        def collect_files(remote_path, local_base):
            ...
            for item in items:
                file_path = item["path"]
                relative_path = file_path.replace(f"disk:/"
                                                  f"{backup_name}", "").lstrip(
                    "/"
                )
                    ...
                    if file_download:
                        files_to_download.append(
                            (file_path, local_path,
                             modification_time_on_cloud, is_gz)
                        )
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(download_file, *args)
    ...
    def backup(self, load_path):
        ...
        folder_name = "{0}_{1}".format(
            os.path.basename(load_path), datetime.now().strftime("%Y_%m_%d")
        )
        ...
        def upload_process(file_path):
            relative_path = os.path.relpath(
                os.path.dirname(file_path), load_path
            ).replace("\\", "/")
            remote_path = (
                folder_name
                if relative_path == "."
                else f"{folder_name}/{relative_path}"
            )
            if relative_path != ".":
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(upload_process, file)
    ...
    def list_of_files_on_backup(self, backup_name):
        ...
        def print_recursive(folder, indent=1):
            response = requests.get(
                f"{self.Y_URL}?path={folder}&fields=_embedded.items."
                f"name,_embedded.items.type,_embedded.items.path",
                headers=self.headers,
            )
            items = response.json().get("_embedded", {}).get("items", [])
            for item in items:
