...
class TestYandexAction(unittest.TestCase):
    ...
    def test_create_folder_when_not_exist(self, mock_put, mock_get):
        ...
        mock_put.assert_called_once_with(
            f"{Y_URL}?path=test_folder", headers=self.yandex_action.headers
        )
    ...
    def test_backup_deleted_successfully(
        self, mock_bar, mock_count_files, mock_delete, mock_get
    ):
        mock_get.side_effect = [MagicMock(status_code=200),
    ...
    def test_compress_file(self, mock_stat, mock_utime):
        ...
        try:
            ...
            self.assertEqual(
                os.path.basename(compressed_name), os.path.basename(
                    expected_gz_name)
            )
            ...
            mock_utime.assert_called_once_with(
                os.path.basename(expected_gz_name), (1234567890, 1234567890)
            )
    ...
    def test_upload_file_does_not_exist_on_cloud(
        self, compress_mock, stat_mock, put_mock, get_mock
    ):
        ...
        with TemporaryDirectory() as tmpdirname:
            ...
            put_mock.assert_called_once_with(
                "https://upload-url", data=buffer.getvalue()
            )

    @patch("source.yandexdrive.YandexAction.os.path.exists")
    @patch("source.yandexdrive.YandexAction.os.stat")
    @patch("source.yandexdrive.YandexAction.requests.get")
    def test_file_up_to_date_no_download(
        self, requests_get_mock, stat_mock, exists_mock
    ):
        os.environ["UNIT_TEST_MODE"] = "1"
    ...
    def test_download_new_file(
        ...
        os.environ["UNIT_TEST_MODE"] = "1"
        with TemporaryDirectory() as tmpdir:
            sys_exit_mock.side_effect = lambda code: print(f"[MOCKED] "
                                                           f"sys.exit({code})")

            backup_name = "testFolder_2025_04_11"
            full_local_path = os.path.join(tmpdir, backup_name)
            exists_mock.side_effect = lambda path: False
            stat_mock.return_value.st_mtime = datetime(2025, 4, 1, 12,
                                                       0, 0).timestamp()
            requests_get_mock.side_effect = [
                MagicMock(
                    status_code=200,
    ...
    def test_list_of_files_on_backup_flat_structure(self, mock_get):
        """
        Тестирует вывод списка файлов в корневом каталоге без
