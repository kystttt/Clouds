...
class GoogleAction:
    ...
    def _get_or_create_folder(self, name, parent_id):
        ...
        :return:
        """
        query = (f"name='{name}' and mimeType='application/"
    ...
    def _upload_file(self, file_path, parent_id):
        ...
        query = (f"name='{file_name}' and '{parent_id}'"
        ...
        if items:
            drive_time = datetime.fromisoformat(
                items[0]["modifiedTime"].replace("Z", "+00:00")
            )
                ...
                if item["mimeType"] == "application/vnd.google-apps.folder":
                    ...
                    if os.path.exists(item_path):
                        local_time = datetime.fromtimestamp(
                            os.path.getmtime(item_path), timezone.utc
                        )
                        if local_time >= drive_time:
                            bar.next()
                            continue
    ...
    def delete_backup_on_cloud(self, backup_name):
        ...
        :param backup_name: str — Название папки, например
         'testFolder_2025_04_11'
        """
